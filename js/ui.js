...
function setTradeMode(mode){
  tradeMode = mode;
  document.getElementById("buySellToggle").innerHTML = `
...
function render() {
  ...
  stocks.forEach((s, i) => {
    ...
    row.innerHTML = `
      ...
      <td>
        ${[1,5,10,20,100].map(n => `<button onclick="trade(${i},${n})">${tradeMode==='buy'?'+':'-'}${n}</button>`).join("")}
      </td>
    `;
...
function trade(stockIndex, amount){
  ...
  if(tradeMode === "buy"){
    ...
    // Sell max is what the player owns
