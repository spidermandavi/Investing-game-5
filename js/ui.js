...
// Popup function with optional graph
function popup(html, data=null, color="#4caf50") {
  const popupEl = document.getElementById("popup");
  const popupContent = document.getElementById("popupContent");

  // Only add canvas if we have graph data
  if(data && data.length > 1){
    popupContent.innerHTML = html + '<canvas id="graphCanvas" style="width:100%;height:200px;margin-top:10px;"></canvas><button id="popupOk">OK</button>';
    setTimeout(()=> drawGraph(data, color), 50);
  } else {
    popupContent.innerHTML = html + '<button id="popupOk">OK</button>';
  }
...
function drawGraph(data, color="#4caf50"){
  ...
  let padding = 40;
  let w = canvas.clientWidth - padding*2;
  let h = canvas.clientHeight - padding*2;
  ...
  ctx.strokeStyle = "#888";
  ctx.lineWidth = 1;
  ctx.beginPath(); ctx.moveTo(padding, padding); ctx.lineTo(padding, padding + h); ctx.stroke(); // Y axis
  ctx.beginPath(); ctx.moveTo(padding, padding + h); ctx.lineTo(padding + w, padding + h); ctx.stroke(); // X axis
...
function render() {
  ...
  // Current player color & text visibility
  ...
  infoBar.innerHTML = `
    ...
    Money: $<span id="moneyVal">${players[currentPlayer].money.toFixed(2)}</span>
  ...
  stocks.forEach((s, i) => {
    ...
    row.innerHTML = `
      ...
      <td class="${changeClass}" id="change${i}">${change.toFixed(2)}</td>
      <td id="owned${i}">${s.owned[currentPlayer]}</td>
      <td>
        ${[1,5,10,20,100].map(n => `
          <button style="background:${tradeMode==='buy'?'#4caf50':'#f44336'}; color:#fff;" onclick="trade(${i},${n})">
            ${tradeMode==='buy'?'+':'-'}${n}
          </button>
        `).join("")}
      </td>
...
function isColorDark(color){
  ...
  let r = parseInt(c.substr(0,2),16); let g = parseInt(c.substr(2,2),16); let b = parseInt(c.substr(4,2),16);
...
function trade(stockIndex, amount){
  ...
  if(tradeMode === "buy"){
    ...
    // Animation
    flashChange(document.getElementById(`owned${stockIndex}`), true);
    floatingText(document.getElementById(`owned${stockIndex}`).parentElement, `+${amount}`, true);
  } else {
    let sellAmount = Math.min(amount, stock.owned[currentPlayer]);
    if(sellAmount > 0){
      sell(stockIndex, sellAmount);
      flashChange(document.getElementById(`owned${stockIndex}`), false);
      floatingText(document.getElementById(`owned${stockIndex}`).parentElement, `-${sellAmount}`, false);
...
function toggleInfo(i){
  let s = stocks[i];
  popup(`<b>${s.name}</b><br>${s.desc}<br><br><i>Price History:</i>`, s.history, "#2196f3");
}
...
function showPlayerInfo(playerIndex){
  ...
  let lastWorth = p.history[p.history.length-1]; // ✅ no delay now
  let prevWorth = p.history[p.history.length-2] ?? p.history[0];
  let changePercent = (((lastWorth-prevWorth)/prevWorth)*100).toFixed(2);
...
function animateNumber(element, start, end, duration=600){
  ...
  function step(timestamp){
    ...
    element.innerText = (start + (end-start)*progress).toFixed(2);
    if(progress < 1) requestAnimationFrame(step);
  }
  requestAnimationFrame(step);
}

function flashChange(element, up=true){
  if(!element) return;
  element.style.transition = "none";
  element.style.color = up ? "#4caf50" : "#ff4c4c";
  setTimeout(()=> element.style.transition = "color 0.6s ease", 50);
  setTimeout(()=> element.style.color = "", 700);
}

function floatingText(parent, text, up=true){
  const span = document.createElement("span");
  span.innerText = text;
  span.style.position = "absolute";
  span.style.left = "50%";
  span.style.transform = "translateX(-50%)";
  span.style.color = up ? "#4caf50" : "#ff4c4c";
  ...
  function animate(){
    y -= 1;
    span.style.top = `${y}px`;
    span.style.opacity = 1 + y/30;
    if(y > -30) requestAnimationFrame(animate);
    else span.remove();
  }
  animate();
}
