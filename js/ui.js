...
// ===== GRAPH DRAWER (High-res & responsive) =====
...
function render() {
  ...
  stocks.forEach((s, i) => {
    ...
    row.innerHTML = `
      ...
      <td>
        ${[1,5,10,20,100].map(n => `
          <button style="background:#4caf50;color:#fff;" onclick="tradeMode='buy';trade(${i},${n})">+${n}</button>
        `).join("")}
      </td>
      <td>
        ${[1,5,10,20,100].map(n => `
          <button style="background:#f44336;color:#fff;" onclick="tradeMode='sell';trade(${i},${n})">-${n}</button>
        `).join("")}
      </td>
    `;
  ...
  setTradeMode(tradeMode); // highlight current mode
}
...
function toggleInfo(i){
  ...
  setTimeout(() => drawGraph(s.history, "#2196f3"), 50); // always latest history
}
...
function showPlayerInfo(playerIndex){
  ...
  const lastWorth = p.history[p.history.length-1]; // 🔥 latest worth
