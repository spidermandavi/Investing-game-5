...
function render() {
  ...
  stocks.forEach((s, i) => {
    ...
    row.innerHTML = `
      ...
      <td>
        ${[1,5,10,20,100].map(n => `
          <button style="background:#4caf50;color:#fff;" onclick="trade(${i},${n})">+${n}</button>
        `).join("")}
      </td>
      <td>
        ${[1,5,10,20,100].map(n => `
          <button style="background:#f44336;color:#fff;" onclick="trade(${i},${n})">-${n}</button>
        `).join("")}
      </td>
    `;
  ...
  setTradeMode(tradeMode); // update toggle highlight
}
...
function trade(stockIndex, amount){
  if(tradeMode === "buy"){
    ...
    let sellAmount = Math.min(amount, stocks[stockIndex].owned[currentPlayer]);
